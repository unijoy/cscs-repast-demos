...
Now let's add some data collection to the model. The data we want to record is the number of Zombies and number of Humans at each timestep. Repast Simphony can record data from agents and Contexts. In this case, we will create a Context and expose the data (the number of Zombies and Humans) in that. 

\begin{enumerate}
\item In the Package Explorer pane, click on the "jzombies" package underneath the src directory. 
\item Click on the "New Java Class" wizard button (Fig.~\ref{fig:class_wizard}).
\item In the Name field, type JZombiesContext
\item In the Superclass field, type repast.simphony.context.DefaultContext
\item Click Finish
\end{enumerate}

An editor pane for JZombiesContext should now be open. If DefaultContext is underlined in yellow, add \texttt{<Object>} after it. The class definition should read\\

\texttt{public class JZombiesContext extends DefaultContext<Object>}\\

Now let's add some code to JZombiesContext to expose the data. Add two methods \texttt{getHumanCount()} and \texttt{getZombieCount()} into JZombiesContext.

\noindent\begin{minipage}[h]{\textwidth}
\vspace{.2in}
\lstset{language=java,caption=Agent Count Methods}
\begin{lstlisting}
...
Because JZombiesContext will be our Context, it will contain our agents. Consequently, we can call \texttt{getObjects(Class)} on it to retrieve a collection of the agents of the specified class type. The size of this collection will then be the number of agents of that type in the Context.

We need to do one more thing to get Repast Simphony to use our JZombiesContext as the context. Open the \texttt{context.xml} file with Eclipse, and add a new child attribute ``class'' to the top context element. \footnote{See ``File Formats'' in the Repast Simphony Reference documentation for more the context.xml file format.} To do that, right click on the context element, then ``Add Attribute'', ``class''. Type jzombies.JZombiesContext for the value. The class attribute here tells Repast Simphony to use the named class as the context class type. The context.xml should now look like fig.~\ref{fig:cxml3}.

\begin{figure}[h]
\begin{center}
\vspace{.2in}
\centerline {
\includegraphics[width=3in]{GettingStartedImages/cxml2.png}
}
\caption{context.xml with Network}
\label{fig:cxml3}
\end{center}
...
Now launch the jzombies Model (exit it and relaunch it if it is already running) via the Eclipse launch button. Data collection is setup up in Repast Simphony by configuring data sets. Think of a data set as a template for producing tabular data where each column represents a particular variable and each row the value of that variable. To create a data set, you specify columns and the source for the data for that column. To add a data set, 

\vspace{.2in}
\begin{enumerate}
\item Right click on `Data Sets'' in the Scenario Tree, and click ``Add Data Set''
\item In the Data Set Editor, type Agent Counts as the Data Set ID, and select JZombiesContext as the Agent Class.
\item Click Next
\item In this panel, you specify columns and the source of the data for those columns. Click Add to add the default ``Simple Mapping'' \footnote{A variety of mappings are available see the user manual for more information.}. This should add the ``Tick'' column and source. This mapping will record the current tick (timestamp) value to the data set.
\item Click Add again. This will add another ``Tick'' mapping but we can change it in place. Click on ``current tick: double'' in the source column (fig.~\ref{fig:data1}), and select ``getHumanCount()'' from the drop down box.
\item Repeat step 5, but choose ``getZombieCount()'' from the drop down. The editor should now look like fig.~\ref{fig:data2}
\item Click Next
...
If we run the model now, data will be recorded but we have not set up anywhere for it to be written to. We will do that now by creating a File Outputter. A File Outputter will write the recorded data out to a file.

\vspace{.2in}
\begin{enumerate}
\item Right click on ``Outputters'' in the Scenario Tree, and click ``Add File Outputter''.
\item The Data Info pane allows us to set some basic properties of the file outputter. If we had created more than one data set we might have to choose it from the Data Set Id combo box, but as we don't the defaults are fine. The data is written in plain text format and the ``Delimiter'' specifies the string used to separate the columns. The File Columns allows us to choose which columns in our data set will be written to the file. We want all of them. Click on each one and then click on the right facing arrow.
\item Click Next
\item The File Properties panel allows us to set some additional file properties: the file name and what meta data to put in the file name. We can leave the defaults as they are.
\item Cick Finish
\end{enumerate}
\vspace{.2in}

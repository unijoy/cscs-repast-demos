...
//	def g  //the gravitational constant
	...
	def setup(){
		...
		vx = Utility.randomFloat(2.0) - 1.0
		vy = Utility.randomFloat(2.0) - 1.0
		xc = getXcor()
		yc = getYcor()
	}
	...
	def random_initial_setup(){
		
	}
	
	def zero_sum_initial_setup(){
	...
	def update_force(){
		//identifies the force vectors acting on this body
//		ask(other(bodies())){
//			sum_its_force_on_me(this)	
//		}
...
//			label = distance+""
//			label = (int)((this.mass * body.mass) / (distance * distance))
//			label = (this.mass * body.mass) / (distance * distance)
//			label = Math.atan(xd*yd)
...
//			label = "x:" + (int)(xc) + " , y:" + (int)yc + " , ang:" + (int)angle + " , sin(angle):"+ fx
		}
//		label = fx + "," + fy
	}
